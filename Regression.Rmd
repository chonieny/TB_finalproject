...
df2 = TB_usage %>%
  select(country, year, hospd_mdr_dur, rcvd_patsup,  rcvd_staff,  rcvd_mdrmgt, rcvd_fld, rcvd_oth, rcvd_patsup, rcvd_prog, rcvd_sld, rcvd_staff, rcvd_tbhiv )
...
lm_model1 = lm(log_hosp ~ year + rcvd_patsup + rcvd_staff + rcvd_mdrmgt + rcvd_fld + rcvd_oth + rcvd_patsup*rcvd_staff, data = TB_usage)

lm_model2 = lm(log_hosp ~ year + rcvd_patsup + rcvd_staff + rcvd_mdrmgt + rcvd_fld + rcvd_oth, data = TB_usage)
```
...
all_lm_model1 = lm(log_hosp ~ year + rcvd_patsup + rcvd_staff + rcvd_mdrmgt + rcvd_fld + rcvd_oth + rcvd_patsup*rcvd_staff, data = TB_regression)

all_lm_model2 = lm(log_hosp ~ year + rcvd_patsup + rcvd_staff + rcvd_mdrmgt + rcvd_fld + rcvd_oth, data = TB_regression)
```
Looking at the regression model for all countries,  all variables are significant but the interaction between **rcvd_patsup** and **rcvd_staff** is not significant.
...
mean(TB_regression$rcvd_patsup)
mean(TB_regression$rcvd_staff)
mean(TB_regression$rcvd_mdrmgt)
mean(TB_regression$rcvd_fld)
mean(TB_regression$rcvd_oth)

newdata = data.frame(year = 2020, rcvd_patsup = 5601390, rcvd_staff = 13714771, rcvd_mdrmgt = 3517401, rcvd_fld = 8813787, rcvd_oth = 7378395) 
...
```
After putting the average funding for each variables, we are  95% confident that the in 2020,  wthe average stay at the hospitals will fall between -0.3268 and 5.75 days